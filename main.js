...
const RECORDS_KEY = "starlight-grand-prix-records-v2";
const DRIFT_STAGES = [
  { level: 0, min: 0, label: "ためよう", color: 0x7df9ff, turbo: 0 },
  { level: 1, min: 0.55, label: "ブルー", color: 0x67e8f9, turbo: 0.58 },
  { level: 2, min: 1.2, label: "ゴールド", color: 0xffd166, turbo: 1.0 },
  { level: 3, min: 2.0, label: "スター", color: 0xff67d4, turbo: 1.5 }
];
...
const QUALITY = (() => {
  ...
  return {
    ...
    pixelRatio: Math.min(cappedDpr, smallTouch ? 0.85 : low ? 1 : 1.25),
...
function collectQaSnapshot() {
  return {
    render: renderer ? { calls: renderer.info.render.calls, triangles: renderer.info.render.triangles, geometries: renderer.info.memory.geometries, textures: renderer.info.memory.textures, pixelRatio: renderer.getPixelRatio(), frameMs: Math.round(frameAverage * 10000) / 10 } : null,
    mode: state.mode,
...
function bindUiTap(button, action) {
  ...
  const run = (event) => {
    ...
    const last = uiTapGuard.get(button) ?? -Infinity;
...
function bindEvents() {
  ...
  const bindHold = (button, key) => {
    ...
    const end = (event) => {
      if (event?.cancelable) event.preventDefault();
      event?.stopPropagation?.();
      if (Number.isFinite(event?.pointerId)) button.releasePointerCapture?.(event.pointerId);
      set(false);
    };
  ...
  const startTouchSteer = (event) => {
    if (event?.cancelable) event.preventDefault();
    event?.stopPropagation?.();
    if (Number.isFinite(event?.pointerId)) dom.touchSteer.setPointerCapture?.(event.pointerId);
...
function courseSectors(course = activeCourse()) {
  const sectors = Array.isArray(course?.sectors) ? course.sectors.filter(Boolean) : [];
  return sectors.length ? sectors : [
    { name: "スタートエリア", tip: "ひかるラインをねらおう" },
    { name: "チャレンジエリア", tip: "カーブ前にじゅんびしよう" },
    { name: "ゴールストレート", tip: "さいごまでスピードをのせよう" }
  ];
}
...
function saveRaceRecord(time, rank, splits = state.sectorSplits) {
  ...
  store[key] = {
    ...
    bestStyleScore: Math.max(state.styleScore || 0, previous?.bestStyleScore || 0),
  ...
  return { previous, record: store[key], isBest };
}

function raceMedal(time, rank) {
  if (!raceMode().timeAttack) {
    if (rank === 1) return { name: "ゴールド", className: "gold", mark: "G" };
    if (rank <= 2) return { name: "シルバー", className: "silver", mark: "S" };
    return { name: "ブロンズ", className: "bronze", mark: "B" };
  }
  const target = Number(activeCourse()?.medalTime || 210) * (activeLapTotal() / 3);
  if (time <= target) return { name: "ゴールド", className: "gold", mark: "G" };
  if (time <= target * 1.16) return { name: "シルバー", className: "silver", mark: "S" };
  return { name: "ブロンズ", className: "bronze", mark: "B" };
}

function driftStageForCharge(charge) {
  let stage = DRIFT_STAGES[0];
  DRIFT_STAGES.forEach((candidate) => {
    if (charge >= candidate.min) stage = candidate;
  });
  return stage;
}
...
function showRaceNotice(kicker, title, message, tone = "info", duration = 1450) {
  ...
  dom.raceNoticeText.textContent = message;
  dom.raceNotice.dataset.tone = tone;
  dom.raceNotice.classList.remove("hidden", "notice-pop");
  void dom.raceNotice.offsetWidth;
  dom.raceNotice.classList.add("notice-pop");
  state.noticeTimer = window.setTimeout(() => dom.raceNotice?.classList.add("hidden"), duration);
}
...
function characterKeyArt(character) {
  const paths = {
    "luna-mimi": "./assets/racer-luna-v53.webp",
    "gamma-bolt": "./assets/racer-gamma-v53.webp",
    "nebi-mist": "./assets/racer-nebi-v53.webp",
    "sora-ranger": "./assets/racer-sora-v53.webp",
    "comet-rin": "./assets/racer-rin-v53.webp"
  };
  ...
  DATA.characters.forEach((character, index) => {
    ...
    button.innerHTML =
      '<span class="racer-set-visuals">' +
        '<img class="racer-keyart" src="' + characterKeyArt(character) + '" alt="" loading="' + (index < 2 ? 'eager' : 'lazy') + '" decoding="async">' +
      '</span>' +
...
function populateCourse() {
  ...
  if (grid) {
    ...
    (DATA.courses || [DATA.course]).forEach((course, index) => {
      ...
      const record = bestRecord(course);
      const recordText = record?.time ? "ベスト " + formatTime(record.time) : "ベスト --:--.-";
...
function initThree() {
  ...
  const hemi = new THREE.HemisphereLight(0xe4f2ff, 0x4b5260, 1.65);
  scene.add(hemi);
...
function buildTrack() {
  ...
  if (terrain) {
    trackInfo.terrain = terrain;
    trackInfo.group.add(terrain);
  }

  addThemedTrackFoundation(trackInfo, theme);
  createTrackRibbon(trackInfo, 0, TRACK_WIDTH, theme.roadColor, theme.roadEmissive, theme.roadOpacity, true);
...
function createTrackRibbon(trackInfo, offset, width, color, emissive, opacity, isRoad, dashEvery = 1, start = 0, end = TRACK_STEPS) {
  const vertices = [];
  const indices = [];
  const uvs = [];
  ...
  for (let n = 0; n < count; n += 1) {
    if (dashEvery > 1 && Math.floor(n / dashEvery) % 2 === 1) continue;
    const a = n * 2;
  ...
  const lunarSurface = themeId === "lunar-crater-run" && isRoad;
  const marsSurface = themeId === "meteor-mining-belt" && isRoad;
  const ringSurface = themeId === "starlight-orbit-ring" && isRoad;
  const iceSurface = themeId === "nebula-drift-stream" && isRoad;
  const materialOptions = {
    color,
    roughness: lunarSurface ? 0.92 : marsSurface ? 0.88 : iceSurface ? 0.44 : isRoad ? 0.62 : 0.42,
    metalness: lunarSurface || marsSurface ? 0.02 : iceSurface ? 0.04 : ringSurface ? 0.18 : isRoad ? 0.16 : 0.03,
    emissive,
    emissiveIntensity: lunarSurface ? 0.018 : marsSurface ? 0.045 : iceSurface ? 0.06 : ringSurface ? 0.055 : isRoad ? 0.035 : 0.34,
    flatShading: false,
    transparent: opacity < 1,
    opacity,
    side: THREE.DoubleSide
...
function addRingTrackBandMarkers(trackInfo, theme) {
  ...
  for (let i = 0; i < TRACK_STEPS; i += (QUALITY.low ? 54 : 24)) {
    ...
    stripe.rotation.set(-sample.grade, Math.atan2(sample.tangent.x, sample.tangent.z), sample.bank, "YXZ");
...
function addLunarStartLine(trackInfo, theme) {
  ...
  sign.position.copy(sample.point).addScaledVector(sample.tangent, 7.5).addScaledVector(sample.normal, TRACK_WIDTH * 0.65);
  sign.position.y += 4.2;
  sign.scale.set(5.4, 1.7, 1);
...
function addStartLine(trackInfo) {
  ...
  const halfSpan = TRACK_WIDTH * 0.55 + 2;
  const postGeo = new THREE.BoxGeometry(1.3, 9.6, 1.6);
  [-1, 1].forEach((side) => {
    const post = new THREE.Mesh(postGeo, gateMat);
    post.position.set(side * halfSpan, 4.8, 0);
    const lamp = new THREE.Mesh(new THREE.BoxGeometry(0.14, 7.8, 1.65), accentMat);
    lamp.position.set(side * (halfSpan - 0.58), 4.8, 0);
    gate.add(post, lamp);
  });
  const bridge = new THREE.Mesh(new THREE.BoxGeometry(halfSpan * 2 + 1.3, 1.15, 1.6), gateMat);
  bridge.position.y = 9.6;
  const strip = new THREE.Mesh(new THREE.BoxGeometry(halfSpan * 2 - 1.3, 0.12, 1.68), accentMat);
  strip.position.y = 9.2;
  ...
  sign.position.set(0, 10.1, 0);
  sign.scale.set(5.8, 1.8, 1);
  gate.add(bridge, strip, sign);
...
function addRails(trackInfo) {
  const theme = trackInfo.theme || courseTheme();
  if (theme.id === "lunar-crater-run") return;
  ...
  const height = isRing ? 1.75 : isIce ? 1.5 : 1.25;
  const geometry = isMars || isIce
    ? new THREE.ConeGeometry(isMars ? 0.44 : 0.38, height, 4)
    : new THREE.BoxGeometry(0.5, height, 0.5);
  const colors = [palette.railLeft || theme.railLeft || "#9eefff", palette.railRight || theme.railRight || "#ffd166"];
  const materials = colors.map((color) => new THREE.MeshStandardMaterial({
    color: new THREE.Color(color).lerp(new THREE.Color(palette.structure || "#526276"), 0.38),
    emissive: color,
    emissiveIntensity: isMars ? 0.1 : 0.16,
    roughness: isMars ? 0.78 : 0.42,
    metalness: isRing ? 0.42 : 0.08,
    flatShading: true
  }));
  const step = QUALITY.low ? 34 : 14;
  for (let i = 0; i < TRACK_STEPS; i += step) {
    const sample = trackInfo.samples[i];
    for (let side = -1; side <= 1; side += 2) {
      const post = new THREE.Mesh(geometry, materials[side > 0 ? 0 : 1]);
      post.position.copy(trackSurfacePoint(sample, side * (TRACK_WIDTH * 0.55 + 0.55), height * 0.5));
      post.rotation.y = Math.atan2(sample.tangent.x, sample.tangent.z) + (isIce ? Math.PI * 0.25 : 0);
...
function createRoadSurfaceTexture(surfaceType = "metal-ring", theme = null) {
  ...
  const toCss = (hex, alpha = 1) => {
    const value = typeof hex === "number"
      ? hex
      : Number.parseInt(String(hex || "#000000").replace("#", ""), 16);
...
function addMarsCourseSet(city, theme = courseTheme()) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    dummy.position.y = terrainHeightAt(THREE, track, dummy.position) + size * 0.36;
    dummy.rotation.set(rand() * Math.PI, rand() * Math.PI, rand() * Math.PI);
...
function addIceCometSet(city, theme = courseTheme()) {
  const crystalMat = new THREE.MeshStandardMaterial({ color: 0xcff8ff, emissive: 0x7df9ff, emissiveIntensity: 0.12, roughness: 0.32, metalness: 0.08, flatShading: true });
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    dummy.position.y = terrainHeightAt(THREE, track, dummy.position) + h * 0.48;
...
function buildEnvironment() {
  ...
  clearSceneryCorridor(THREE, city, track);
  scene.add(city);
...
function createTextSprite(label, color = 0xffffff, opacity = 0.9) {
  ...
  texture.generateMipmaps = false;
  texture.minFilter = THREE.LinearFilter;
  return new THREE.Sprite(new THREE.SpriteMaterial({ map: texture, transparent: true, opacity, depthWrite: false, toneMapped: false }));
}
function addLunarCourseSet(city, theme = courseTheme()) {
  ...
  for (let i = 0; i < craterCount; i += 1) {
    ...
    dummy.position.copy(sample.point).addScaledVector(sample.normal, lateral);
    dummy.position.y = terrainHeightAt(THREE, track, dummy.position) + 0.035;
    dummy.rotation.set(-Math.PI / 2, 0, rand() * Math.PI);
    ...
    dummy.position.y = terrainHeightAt(THREE, track, dummy.position) + 0.25 + rand() * 0.45;
    dummy.rotation.set(rand() * Math.PI, rand() * Math.PI, rand() * Math.PI);
  ...
  const earth = createEarth(THREE);
  const startView = track.samples[layoutIndex(18)];
  earth.position.copy(startView.point).addScaledVector(startView.tangent, 265).addScaledVector(startView.normal, 94);
  earth.position.y += 62;
  city.add(earth);
...
function addLunarHeroCrater(city, index, offset, size) {
  ...
  crater.position.y = terrainHeightAt(THREE, track, crater.position) + 0.05;
...
function addLunarBaseCluster(city, theme, layout, offset, primary) {
  ...
  group.position.copy(sample.point).addScaledVector(sample.normal, offset);
  group.position.y = terrainHeightAt(THREE, track, group.position) - 0.1;
  group.rotation.y = Math.atan2(sample.tangent.x, sample.tangent.z) + (offset > 0 ? -Math.PI * 0.5 : Math.PI * 0.5);
  city.add(group);
}
...
function addLunarLander(city, theme, layout, offset) {
  ...
  group.position.copy(sample.point).addScaledVector(sample.normal, offset);
  group.position.y = terrainHeightAt(THREE, track, group.position);
...
function createRouteBeacon() {
  ...
  routeBeacon.scale.set(4.8, 2.4, 1);
...
function calculateRouteCue(racer = player) {
  ...
  const lookAhead = 42 + Math.abs(racer.speed || 0) * 0.9;
  const nearIndex = trackIndexAtDistance(racer.trackIndex, 12);
  const targetIndex = trackIndexAtDistance(racer.trackIndex, lookAhead);
  ...
  const distance = Math.round(lookAhead / 10) * 10;
  routeCue = {
    direction,
    arrow,
...
function trackIndexAtDistance(startIndex, distance) {
  let index = (Math.round(startIndex) + TRACK_STEPS) % TRACK_STEPS;
  let remaining = Math.abs(distance);
  const direction = distance < 0 ? -1 : 1;
  for (let step = 0; step < TRACK_STEPS && remaining > 0; step++) {
    const next = (index + direction + TRACK_STEPS) % TRACK_STEPS;
    remaining -= track.samples[direction > 0 ? index : next].segmentLength || 1;
    index = next;
  }
  return index;
}
...
function updateRouteBeacon() {
  ...
  routeBeacon.position.y += cue.crest ? 6 : 4.6;
  routeBeacon.position.x += sample.normal.x * (TRACK_WIDTH * 0.5 + 2);
  routeBeacon.position.z += sample.normal.z * (TRACK_WIDTH * 0.5 + 2);
...
function resetRace() {
  ...
  state.styleScore = 0;
  state.raceMistakes = 0;
  state.overtakes = 0;
  state.duelActive = false;
  state.duelRivalId = "";
  state.lastDuelNoticeAt = -99;
  lastCoachKey = "";
  lastHudLap = 0;
  lastHudRank = 1;
  state.winner = null;
...
function cpuRaceProfile(character, gridSlot) {
  const profiles = [
    { name: "ラインよみ", lane: 0.38, aggression: 0.72, corner: 1.05, drift: 1.08, boost: 0.94 },
    { name: "パワー", lane: 0.62, aggression: 1.14, corner: 0.94, drift: 0.88, boost: 1.02 },
    { name: "フロート", lane: 0.48, aggression: 0.82, corner: 1.08, drift: 1.14, boost: 1.06 },
    { name: "バランス", lane: 0.52, aggression: 0.96, corner: 1.0, drift: 1.0, boost: 1.0 },
    { name: "スプリント", lane: 0.72, aggression: 1.18, corner: 0.9, drift: 0.94, boost: 1.16 }
  ];
  const index = Math.max(0, DATA.characters.indexOf(character));
  return profiles[index % profiles.length] || profiles[gridSlot % profiles.length];
}
...
function createRacer(id, character, kart, gridSlot, laneOffset, isPlayer) {
  ...
  const contactShadow = createContactShadow(THREE, profile.width, profile.length);
  ...
  return {
    ...
    laneOffset,
    ai: {
      profile: cpuRaceProfile(character, gridSlot),
...
function createKartModel(character, kart, isPlayer) {
  return createRaceVehicle(THREE, character, kart, getKartVisualProfile(kart), { low: QUALITY.low, isPlayer });
}

function createLegacyKartModel(character, kart, isPlayer) {
  const group = new THREE.Group();
...
function createPrismGeometry(width, height, length, frontScale, rearScale) {
  ...
  const indices = [0, 2, 1, 0, 3, 2, 4, 5, 6, 4, 6, 7, 0, 5, 4, 0, 1, 5, 1, 6, 5, 1, 2, 6, 2, 7, 6, 2, 3, 7, 3, 4, 7, 3, 0, 4];
  const geometry = new THREE.BufferGeometry();
...
async function startCountdown() {
  ...
  dom.app?.classList.add("is-racing", "is-countdown");
  ...
  for (const label of ["3", "2", "1", "スタート！"]) {
    state.countdown = Number(label) || 0;
    dom.countdown.textContent = label;
    pulseCountdown(label);
    await wait(680);
  }
  dom.countdown.classList.add("hidden");
  dom.app?.classList.remove("is-countdown");
  state.mode = "racing";
  state.time = 0;
  const startSuccess = player && !state.startEarly && state.startCharge >= 0.14;
  if (startSuccess) {
    state.startResult = "success";
    player.speed = Math.max(player.speed, 16);
    player.miniTurboTimer = Math.max(player.miniTurboTimer, 1.12);
    spawnShockwave(player.position, colorToHex(player.kart.boostColor || player.character.boostColor || "#ffd166"), 5.6);
    playAudio("boost", 0.5);
    gameHaptic([12, 18, 28]);
    awardRaceSkill(player, "ナイススタート", 100);
    showRaceNotice("ナイススタート", "スタートダッシュ！", "「1」で すすむを押せたよ", "boost", 1500);
  } else if (player && state.startEarly) {
    state.startResult = "early";
    player.stunTimer = Math.max(player.stunTimer, 0.42);
    showRaceNotice("フライング", "ちょっと早い！", "つぎは「1」で すすむを押そう", "warning", 1500);
  } else {
    state.startResult = "normal";
    const first = courseSectors()[0];
    showRaceNotice("セクター 1/3", first.name, first.tip, "info", 1500);
  }
}

function updateCountdownStart(dt) {
  if (state.mode !== "countdown") return;
  if (input.accel && state.countdown > 1 && state.countdown <= 3) state.startEarly = true;
  if (input.accel && state.countdown === 1 && !state.startEarly) state.startCharge = Math.min(0.8, state.startCharge + dt);
  const pct = state.startEarly ? 100 : clamp(state.startCharge / 0.5, 0, 1) * 100;
  dom.driftReadout?.classList.toggle("start-ready", state.countdown === 1 && !state.startEarly);
  dom.driftReadout?.classList.toggle("start-early", state.startEarly);
  if (dom.driftStage) dom.driftStage.textContent = state.startEarly ? "はやい！" : state.countdown === 1 ? "いま！" : "「1」で押す";
  if (dom.driftMeterFill) dom.driftMeterFill.style.width = pct + "%";
}
...
function renderRaceResults(winner, playerRank) {
  ...
  const saved = saveRaceRecord(state.finishTime, playerRank);
  const medal = raceMedal(state.finishTime, playerRank);
  const recordLabel = saved.isBest ? "しんきろく！" : "ベスト " + formatTime(saved.record.time);
  const fastestLap = state.lapTimes.length ? Math.min(...state.lapTimes) : state.finishTime;
  const grade = flowGrade();
  const gradeCopy = grade === "S" ? "スター走行" : grade === "A" ? "エース走行" : grade === "B" ? "ナイス走行" : "チャレンジ走行";
  const award = '<div class="result-award award-' + medal.className + '">' +
    '<span class="medal-mark">' + medal.mark + '</span>' +
    '<span><small>今回のメダル</small><strong>' + medal.name + '</strong><em>' + recordLabel + '</em></span>' +
  '</div>';
  const flowAward = '<div class="result-flow-award grade-' + grade.toLowerCase() + '">' +
    '<span class="flow-mark">' + grade + '</span>' +
    '<span><small>ドライビング FLOW</small><strong>' + gradeCopy + '</strong><em>' + state.styleScore + 'ポイント / ミス ' + state.raceMistakes + 'かい / ぬいた回数 ' + state.overtakes + 'かい</em></span>' +
  '</div>';
  if (mode.timeAttack) {
    dom.resultTitle.textContent = saved.isBest ? "しんきろく！" : "タイムアタック かんそう！";
    dom.resultStats.innerHTML = playerCard + award + flowAward +
...
function updateAdaptiveQuality(frameSeconds, menuScene) {
  ...
  frameAverage = frameAverage * 0.96 + Math.min(frameSeconds, 0.2) * 0.04;
  qualitySampleTime += Math.min(frameSeconds, 0.1);
  // Only lower resolution after sustained load; keep changes infrequent.
  if (qualitySampleTime > 5 && state.time > 4) {
    if (frameAverage > 0.034 && adaptiveScale > 0.8) {
      adaptiveScale = Math.max(0.8, adaptiveScale - 0.1);
      renderer.setPixelRatio(QUALITY.pixelRatio * adaptiveScale);
    }
    qualitySampleTime = 0;
  }
...
function updateRacer(racer, dt) {
  ...
  racer.mistakeLock = Math.max(0, (racer.mistakeLock || 0) - dt);
  ...
  if (racer.driftActive) {
    racer.driftCharge = Math.min(2.6, racer.driftCharge + dt);
    const chargedStage = driftStageForCharge(racer.driftCharge);
    if (chargedStage.level > (racer.driftStage || 0)) {
      racer.driftStage = chargedStage.level;
      spawnBurst(racer.position, chargedStage.color, racer.isPlayer ? 8 : 3, 0.72);
      if (racer.isPlayer) {
        playAudio("countdown", chargedStage.level);
        gameHaptic(8 + chargedStage.level * 4);
      }
    }
    if (!controls.drift || racer.speed < maxSpeed * 0.2) {
      const releaseStage = DRIFT_STAGES[racer.driftStage || 0] || DRIFT_STAGES[0];
      if (releaseStage.turbo > 0) {
        racer.miniTurboTimer = Math.max(racer.miniTurboTimer, releaseStage.turbo);
        racer.speed = Math.max(racer.speed, maxSpeed * (0.72 + releaseStage.level * 0.05));
        spawnBurst(racer.position, releaseStage.color, racer.isPlayer ? 12 : 4, 1.05 + releaseStage.level * 0.18);
        if (racer.isPlayer) {
          playAudio("boost", 0.36 + releaseStage.level * 0.08);
          gameHaptic(10 + releaseStage.level * 5);
          awardRaceSkill(racer, releaseStage.label + "ドリフト", releaseStage.level === 3 ? 320 : releaseStage.level === 2 ? 180 : 80);
          showRaceNotice("ドリフトダッシュ", releaseStage.label + " ダッシュ！", releaseStage.level === 3 ? "さいこうのタイミング！" : "もっとためると長くダッシュ", "boost", 950);
        }
      }
  ...
  if (racer.contactShadow) {
    racer.contactShadow.position.copy(racer.position);
    racer.contactShadow.position.y += 0.055;
    racer.contactShadow.rotation.set(-roadPitch, racer.yaw, roadBank, "YXZ");
    racer.contactShadow.material.opacity = 0.66 / (1 + racer.jumpHeight * 0.18);
    racer.contactShadow.scale.setScalar(1 + Math.min(racer.jumpHeight, 8) * 0.06);
  }
...
function readCpuControls(racer, dt) {
  ...
  const profile = racer.ai.profile || cpuRaceProfile(racer.character, 0);
  const theme = track?.theme || courseTheme();
  ...
  targetOffset += Math.sin(state.time * (0.42 + profile.aggression * 0.16) + racer.ai.noise) * TRACK_WIDTH * 0.025 * profile.lane;
  ...
  racers.forEach((other) => {
    ...
    if (ahead > -1.5 && ahead < 16 && Math.abs(side) < 5.8) {
      if (!racer.ai.overtakeSide || (racer.ai.overtakeTimer || 0) <= 0) {
        racer.ai.overtakeSide = -Math.sign(side || racer.ai.targetOffset || (rand() - 0.5));
        racer.ai.overtakeTimer = 1.1 + profile.aggression * 0.7;
      }
      const escapeSide = -racer.ai.overtakeSide;
  ...
  racer.ai.overtakeTimer = Math.max(0, (racer.ai.overtakeTimer || 0) - dt);
  racer.ai.laneShift = approach(racer.ai.laneShift || 0, 0, dt * (1.1 + profile.aggression * 0.3));
  ...
  let targetSpeed = baseMax * (1 - cornerSlowdown / Math.max(0.82, profile.corner));
  if (icyCourse && curve > 0.14) targetSpeed *= 0.91;
  if (edgeAmount > 0.01) targetSpeed *= 1 - edgeAmount * 0.56;
  if (upcomingBoost && boostUsage * profile.boost > 0.62 && edgeAmount < 0.28) targetSpeed *= 1.1 + profile.boost * 0.04;
  ...
  const drift = racer.ai.recoveryTimer <= 0 && edgeAmount < 0.34 && curve > (icyCourse ? 0.16 : (0.22 + mistakeRate * 0.18) / profile.drift) && Math.abs(steer) > (icyCourse ? 0.24 : 0.32) && racer.speed > baseMax * 0.38;
  ...
  for (const panel of boostPanels) {
    if (indexDistance(nearest.index, panel.userData.index) < 5 && Math.abs(nearest.lateral) < TRACK_WIDTH * 0.38) {
      ...
      if (freshBoost) {
        ...
        spawnBoostTrail(racer);
        if (racer.isPlayer) {
          awardRaceSkill(racer, "ブーストライン", 70);
...
function flowGrade(score = state.styleScore, mistakes = state.raceMistakes) {
  if (score >= 2200 && mistakes <= 1) return "S";
  if (score >= 1400) return "A";
  if (score >= 700) return "B";
  return "C";
}

function awardRaceSkill(racer, label, points, options = {}) {
  if (!racer?.isPlayer || state.mode !== "racing") return 0;
  const previousGrade = flowGrade();
  const useChain = options.chain !== false;
  if (useChain) {
    racer.skillChain = (racer.skillChain || 0) + 1;
    racer.skillChainTimer = Math.max(racer.skillChainTimer || 0, options.duration || 5.2);
    state.bestSkillChain = Math.max(state.bestSkillChain || 0, racer.skillChain);
  }
  const multiplier = 1 + Math.min(Math.max((racer.skillChain || 1) - 1, 0), 5) * 0.15;
  const gained = Math.max(1, Math.round(points * multiplier));
  state.styleScore = Math.max(0, (state.styleScore || 0) + gained);
  const nextGrade = flowGrade();
  if (dom.raceFlow) {
    dom.raceFlow.dataset.flow = "FLOW " + nextGrade + " / " + state.styleScore;
    dom.raceFlow.classList.remove("flow-hit");
    void dom.raceFlow.offsetWidth;
    dom.raceFlow.classList.add("flow-hit");
  }
  if (nextGrade !== previousGrade) {
    showRaceNotice("FLOW RANK UP", nextGrade + "グレード", label + "で走りがつながった！", nextGrade === "S" ? "final" : "boost", 1100);
    playAudio("skill", nextGrade === "S" ? 3 : 2);
    gameHaptic(nextGrade === "S" ? [10, 18, 28] : 10);
  }
  return gained;
}

function registerRaceMistake(racer) {
  if (!racer?.isPlayer || (racer.mistakeLock || 0) > 0) return;
  racer.mistakeLock = 1.2;
  state.raceMistakes = (state.raceMistakes || 0) + 1;
...
function handleLandingGrade(racer, nearest) {
  ...
  if (perfect || clean) {
    ...
    awardRaceSkill(racer, perfect ? "パーフェクト着地" : "ナイス着地", perfect ? 280 : 120);
...
function hitRacer(racer, item, owner, stun = 1.25) {
  ...
  if (racer.isPlayer || owner?.isPlayer) {
    if (racer.isPlayer) registerRaceMistake(racer);
...
function spawnBurst(origin, color, count, power) {
  ...
  for (let i = 0; i < safeCount; i += 1) {
    const shard = i % 4 === 0;
    const particle = burstPool.pop() || {
      mesh: new THREE.Mesh(new THREE.TetrahedronGeometry(0.2, 0), new THREE.MeshBasicMaterial({ transparent: true, blending: THREE.AdditiveBlending, depthWrite: false })),
      velocity: new THREE.Vector3(), pooled: true
    };
    const mesh = particle.mesh;
    mesh.material.color.set(color);
    mesh.material.opacity = 0.78;
    mesh.scale.setScalar(0.6 + rand() * 0.8);
    mesh.position.copy(origin);
    mesh.position.y += 0.4 + rand() * 1.6;
    mesh.rotation.set(rand() * Math.PI, rand() * Math.PI, rand() * Math.PI);
    particle.velocity.set(rand() - 0.5, rand() * 0.9, rand() - 0.5).normalize().multiplyScalar(power * (3 + rand() * 7));
    particle.life = 0.35 + rand() * 0.45;
    particle.maxLife = 0.8;
    particle.spin = shard ? (rand() - 0.5) * 10 : 0;
    scene.add(mesh);
...
function releaseParticle(particle) {
  ...
  if (particle.pooled && burstPool.length < QUALITY.particleCap) burstPool.push(particle);
  else disposeObject3D(particle.mesh);
}
...
function updateHud() {
  ...
  if (rankDelta !== 0) {
    ...
    if (rankDelta < 0 && state.time > 3) {
      const passes = Math.abs(rankDelta);
      state.overtakes += passes;
      awardRaceSkill(player, "追い抜き", 220 * passes);
...
function updateRivalGap() {
  ...
  const rivals = racers.filter((racer) => racer !== player && !racer.finished);
  if (!rivals.length) {
    dom.rivalGap.textContent = (player.skillChain || 0) > 1 ? player.skillChain + "れんぞく / FLOW " + flowGrade() : "じぶんのベストへ";
    dom.hud?.classList.remove("is-duel");
    state.duelActive = false;
    state.duelRivalId = "";
    return;
  }
  ...
  const rivalName = displayName(nearest.racer.character);
  const relation = nearest.gap >= 0 ? "前の" + rivalName + "まで " : rivalName + "が後ろ ";
  const chain = (player.skillChain || 0) > 1 ? " / " + player.skillChain + "れんぞく" : " / FLOW " + flowGrade();
  dom.rivalGap.textContent = relation + meters + "m" + chain;
  dom.rivalGap.dataset.relation = nearest.gap >= 0 ? "ahead" : "behind";

  const wasDuel = state.duelActive;
  const previousRival = state.duelRivalId;
  const close = state.mode === "racing" && meters <= 45;
  state.duelActive = close;
  state.duelRivalId = close ? nearest.racer.id : "";
  dom.hud?.classList.toggle("is-duel", close);
  if (close) {
    dom.hud?.style.setProperty("--duel-color", nearest.racer.kart?.boostColor || nearest.racer.character?.boostColor || "#ffd166");
    if ((!wasDuel || previousRival !== nearest.racer.id) && state.time - state.lastDuelNoticeAt > 8) {
      state.lastDuelNoticeAt = state.time;
      showRaceNotice("デッドヒート", rivalName + "とせっせん！", "ぬけるきょりまで " + meters + "m", "final", 950);
      playAudio("skill", 2);
      gameHaptic(8);
    }
  }
...
function updateRaceFlow() {
  ...
  dom.driftReadout.dataset.stage = String(stage.level);
  dom.driftReadout.classList.toggle("is-active", player.driftActive);
  dom.driftReadout.classList.remove("start-ready", "start-early");
  ...
  if (state.lastSectorKey && key !== state.lastSectorKey && state.mode === "racing" && player.startedLap) {
    ...
    if (Number.isFinite(state.paceDelta) && state.paceDelta <= 0) awardRaceSkill(player, "セクターベスト", 140);
    const missionText = sector.challenge ? sector.challenge + " / " : "";
    showRaceNotice("セクター " + (sectorIndex + 1) + "/" + sectors.length, sector.name, missionText + sector.tip + paceText, Number.isFinite(state.paceDelta) && state.paceDelta <= 0 ? "boost" : "info", 1400);
  ...
  if (dom.paceLabel) dom.paceLabel.textContent = "ベストとの差 / FLOW " + flowGrade();
  if (dom.paceValue) {
...
function updateRaceCoach(boosting, speedKmh) {
  ...
  const sectors = courseSectors();
  const sectorIndex = Math.min(sectors.length - 1, Math.floor(clamp((player.trackIndex || 0) / TRACK_STEPS, 0, 0.999) * sectors.length));
  const sector = sectors[sectorIndex] || sectors[0];
  let key = "mission-" + sectorIndex;
  let title = sector?.challenge || "ひかる線を走ろう";
  let text = sector?.tip || "コースのまんなかをねらおう";
  let tone = "line";
...
function updateCamera(dt) {
  if (!player) return;
  const forward = cameraForward.set(Math.sin(player.yaw), 0, Math.cos(player.yaw));
  const right = cameraRight.set(Math.cos(player.yaw), 0, -Math.sin(player.yaw));
  ...
  const behind = 18.5 + speed01 * 3.2 + (boosting ? 2.6 : 0) + portraitFraming * 7.5;
  const height = 7.8 + speed01 * 0.8 + portraitFraming * 2;
  const targetPos = cameraDesired.copy(player.position)
    .addScaledVector(forward, -behind)
    .addScaledVector(right, steerLean * 1.45 + driftLean * 1.1);
  targetPos.y += height + player.jumpHeight * 0.45;
  const behindSample = track.samples[trackIndexAtDistance(player.trackIndex, -behind)];
  targetPos.y = Math.max(targetPos.y, behindSample.point.y + 4.6);
  if (cameraShake > 0) {
  ...
  cameraTarget.copy(player.position).addScaledVector(forward, 14 + speed01 * 10);
  cameraTarget.addScaledVector(right, player.driftActive ? driftLean * 3.0 : steerLean * 1.55);
  const aheadSample = track.samples[trackIndexAtDistance(player.trackIndex, 25 + speed01 * 18)];
  cameraTarget.y += 1.5 + (aheadSample.point.y - player.position.y) * 0.58 + player.jumpHeight * 0.22;
  const roll = clamp(-steerLean * 0.055 + driftLean * 0.105, -0.16, 0.16);
  camera.up.set(0, 1, 0);
  camera.lookAt(cameraTarget);
  camera.rotateZ(roll * 0.45);
  camera.fov = approach(camera.fov, 55 + speed01 * 7 + (boosting ? 5 : 0) + portraitFraming * 4, 24 * dt);
  ...
  speedLines.forEach((line, index) => {
    ...
    line.material.opacity = boosting ? 0.28 : clamp((speed01 - 0.85) * 1.2, 0, 0.12);
